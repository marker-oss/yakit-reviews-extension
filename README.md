...
3. **CORS настроен?** Ошибки `blocked by CORS policy` в консоли — в `.env` не
   указан (или указан с опечаткой/без www-варианта) `REVIEWS_SHOP_ORIGIN`.
   Значение должно точно совпадать с адресом магазина, включая `https://`.
4. **Данные существуют?** `https://сервер-отзывов/reviews-data/links.json`
   должен открываться в браузере. 404 — каталог ещё не обходился или не нажата
   «Опубликовать»: Маркетплейсы → «Обновить каталог товаров», затем публикация.
