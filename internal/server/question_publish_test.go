...
func TestPublishQuestionAnswerSuccess(t *testing.T) {
	...
	s.cfg.ResolveQuestionPublisher = func(_ context.Context, _ string) (marketplace.QuestionAnswerPublisher, error) { return pub, nil }
...
func TestPublishQuestionAnswerFailure(t *testing.T) {
	s := newAuthTestServer(t)
	ctx := context.Background()
	s.cfg.ResolveQuestionPublisher = func(_ context.Context, _ string) (marketplace.QuestionAnswerPublisher, error) {
		return &fakeQuestionPublisher{err: errors.New("network error")}, nil
	}
...
// TestPublishQuestionAnswerResolverFreshness proves each publication resolves
// the publisher afresh: A on first call, B on second.
func TestPublishQuestionAnswerResolverFreshness(t *testing.T) {
	s := newAuthTestServer(t)
	ctx := context.Background()
	pubA, pubB := &fakeQuestionPublisher{}, &fakeQuestionPublisher{}
	calls := 0
	s.cfg.ResolveQuestionPublisher = func(_ context.Context, _ string) (marketplace.QuestionAnswerPublisher, error) {
		calls++
		if calls == 1 {
	...
	for _, ext := range []string{"q-a", "q-b"} {
		q, err := s.store.UpsertQuestion(ctx, store.QuestionInput{
			Marketplace: "wb", ExternalQuestionID: ext,
			AuthorName: "А", Text: "вопрос", CreatedAtMP: testTime(),
		})
		if err != nil {
			t.Fatal(err)
		}
		if err := s.store.SetQuestionAnswer(ctx, q.ID, "ответ"); err != nil {
			t.Fatal(err)
		}
		stored, _ := s.store.QuestionByID(ctx, q.ID)
		s.publishQuestionAnswer(ctx, stored)
	}
...
// TestPublishQuestionAnswerResolverErrorRecordsFailed: resolver error →
// "failed" with actionable message, not "unsupported".
func TestPublishQuestionAnswerResolverErrorRecordsFailed(t *testing.T) {
	s := newAuthTestServer(t)
	ctx := context.Background()
	s.cfg.ResolveQuestionPublisher = func(_ context.Context, _ string) (marketplace.QuestionAnswerPublisher, error) {
	...
	q, err := s.store.UpsertQuestion(ctx, store.QuestionInput{
		Marketplace: "wb", ExternalQuestionID: "q-err",
		AuthorName: "А", Text: "вопрос", CreatedAtMP: testTime(),
	})
	if err != nil {
		t.Fatal(err)
	...
	rows, _ := s.store.QuestionsNeedingAnswerPublish(ctx)
	s.publishQuestionAnswer(ctx, rows[len(rows)-1])

	got, _ := s.store.QuestionByID(ctx, q.ID)
	if got.AnswerPublishState == nil || *got.AnswerPublishState != "failed" {
		t.Fatalf("state = %v, want failed", got.AnswerPublishState)
	}
}
