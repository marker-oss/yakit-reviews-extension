...
func (s *Server) publishReply(ctx context.Context, review store.Review) {
	...
	if s.cfg.ResolveReplyPublisher == nil || !s.replyPublishEnabled(ctx, review.Marketplace) {
		s.setUnsupported(ctx, id)
		return
	}
	pub, err := s.cfg.ResolveReplyPublisher(ctx, review.Marketplace)
	if err != nil {
		msg := err.Error()
		_ = s.store.SetReplyPublishState(ctx, id, "failed", &msg, nil)
		s.logger.Warn("resolve reply publisher failed", "review", id, "marketplace", review.Marketplace, "error", err)
		return
	}
	text := ""
