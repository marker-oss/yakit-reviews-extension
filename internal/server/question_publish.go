...
func (s *Server) publishQuestionAnswer(ctx context.Context, q store.Question) {
	...
	if s.cfg.ResolveQuestionPublisher == nil || !s.replyPublishEnabled(ctx, q.Marketplace) {
		s.setQuestionUnsupported(ctx, id)
		return
	}
	pub, err := s.cfg.ResolveQuestionPublisher(ctx, q.Marketplace)
	if err != nil {
		msg := err.Error()
		_ = s.store.SetQuestionAnswerPublishState(ctx, id, "failed", &msg, nil)
		s.logger.Warn("resolve question publisher failed", "question", id, "marketplace", q.Marketplace, "error", err)
		return
	}
	text := ""
	if q.AnswerText != nil {
