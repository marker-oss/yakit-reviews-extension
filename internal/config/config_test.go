...
// (no signature verification), so tests can construct these directly.
...
func TestValidateMarketplaceCredentialsWBPersonalToken(t *testing.T) {
	cfg := Config{
		Marketplaces: MarketplaceConfig{
			WB: WBConfig{
				Enabled: true,
				Token:   wbJWT(t, map[string]any{"acc": 3, "exp": time.Now().Add(time.Hour).Unix()}),
			},
		},
	}
	if err := cfg.ValidateMarketplaceCredentials(MarketplaceWB); err != nil {
		t.Fatalf("valid personal token rejected: %v", err)
	}
}

func TestValidateMarketplaceCredentialsWBBaseTokenRejected(t *testing.T) {
	cfg := Config{
		Marketplaces: MarketplaceConfig{
			WB: WBConfig{
				Enabled: true,
				Token:   wbJWT(t, map[string]any{"acc": 1, "exp": time.Now().Add(time.Hour).Unix()}),
			},
		},
	}
	err := cfg.ValidateMarketplaceCredentials(MarketplaceWB)
	if !errors.Is(err, wbtoken.ErrPersonalTokenRequired) {
		t.Fatalf("base token error = %v, want ErrPersonalTokenRequired", err)
	}
}

func TestValidateMarketplaceCredentialsDisabledWBIgnoresTokenValidity(t *testing.T) {
	cfg := Config{
		Marketplaces: MarketplaceConfig{
			WB: WBConfig{
				Enabled: false,
	...
	if err := cfg.ValidateMarketplaceCredentials(""); err != nil {
		t.Fatalf("disabled WB with invalid token should be ignored: %v", err)
	}
}
