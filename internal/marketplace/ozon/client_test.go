...
func TestFetchReviewsResolvesSellerArticleViaOfferID(t *testing.T) {
	...
	client := NewWithHTTPClient(config.OzonConfig{ClientID: "client", APIKey: "key"}, "https://api-seller.test", httpClient, 100, nil)
...
func TestProductArticlesPaginates(t *testing.T) {
	...
	client := NewWithHTTPClient(config.OzonConfig{ClientID: "client", APIKey: "key"}, "https://api-seller.test", httpClient, 2, nil)
...
func TestFetchQuestionsMapsOzonResponse(t *testing.T) {
	...
	c := NewWithHTTPClient(config.OzonConfig{ClientID: "cid", APIKey: "key"}, srv.URL, srv.Client(), 100, nil)
...
func TestPublishQuestionAnswerOzon(t *testing.T) {
	...
	c := NewWithHTTPClient(config.OzonConfig{ClientID: "cid", APIKey: "key"}, srv.URL, srv.Client(), 0, nil)
...
func TestFetchReviewsRetriesOzonThrottle(t *testing.T) {
	var call int
	var bodies []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.URL.Path == "/v3/product/list" {
			w.WriteHeader(http.StatusForbidden)
			_, _ = w.Write([]byte(`{"code":7,"message":"no role"}`))
			return
		}
		call++
		b, _ := io.ReadAll(r.Body)
		...
		_, _ = w.Write([]byte(`{
			"reviews": [
				{"id": "r-1", "sku": "SKU-1", "rating": 5, "published_at": "2026-02-02T10:00:00Z"}
			],
			"has_next": false,
			"last_id": ""
		}`))
	...
	c := NewWithHTTPClient(config.OzonConfig{ClientID: "cid", APIKey: "key"}, srv.URL, srv.Client(), 100, nil)
	reviews, _, err := c.FetchReviews(context.Background(), time.Time{}, "")
	if err != nil {
		t.Fatalf("fetch reviews: %v", err)
	}
	if call != 2 {
	...
	if len(bodies) != 2 || bodies[0] != bodies[1] {
		t.Fatalf("bodies differ across attempts: %+v", bodies)
	}
	if len(reviews) != 1 || reviews[0].ExternalReviewID != "r-1" {
		t.Fatalf("reviews = %+v", reviews)
	}
}

func TestPublishQuestionAnswerDoesNotRetryOzonThrottle(t *testing.T) {
	var call int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		call++
		w.Header().Set("Retry-After", "1")
	...
	c := NewWithHTTPClient(config.OzonConfig{ClientID: "cid", APIKey: "key"}, srv.URL, srv.Client(), 0, nil)
	err := c.PublishQuestionAnswer(context.Background(), "qid-1", "sku-1", "x")
	if err == nil {
		t.Fatal("expected error on throttle")
	}
	if call != 1 {
