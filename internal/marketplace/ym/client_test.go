...
func TestFetchReviewsMapsYMResponse(t *testing.T) {
	...
	client := NewWithHTTPClient(
		...
		"https://api.partner.test", httpClient, 50, nil,
	)
...
func TestFetchReviewsPaginatesWithPageToken(t *testing.T) {
	...
	client := NewWithHTTPClient(config.YMConfig{APIKey: "key", BusinessID: "777"}, "https://api.partner.test", httpClient, 50, nil)
...
func TestPublishReplyPostsComment(t *testing.T) {
	...
	c := NewWithHTTPClient(config.YMConfig{APIKey: "key", BusinessID: "42"}, srv.URL, srv.Client(), 0, nil)
...
func TestPublishReplyBadID(t *testing.T) {
	c := NewWithHTTPClient(config.YMConfig{APIKey: "key", BusinessID: "42"}, "http://unused", nil, 0, nil)
...
func TestFetchReviewsRetriesYMThrottle(t *testing.T) {
	var call int
	var bodies []string
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		call++
		...
		_, _ = w.Write([]byte(`{
			"status": "OK",
			"result": {"feedbacks": [], "paging": {"nextPageToken": ""}}
		}`))
	}))
	defer srv.Close()

	c := NewWithHTTPClient(config.YMConfig{APIKey: "key", BusinessID: "777"}, srv.URL, srv.Client(), 50, nil)
	_, _, err := c.FetchReviews(context.Background(), time.Time{}, "")
	if err != nil {
		t.Fatalf("fetch reviews: %v", err)
	}
	if call != 2 {
...
func TestPublishReplyDoesNotRetryYMThrottle(t *testing.T) {
	var call int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		call++
		w.Header().Set("Retry-After", "1")
	...
	c := NewWithHTTPClient(config.YMConfig{APIKey: "key", BusinessID: "42"}, srv.URL, srv.Client(), 0, nil)
	err := c.PublishReply(context.Background(), "1001", "x")
	if err == nil {
