...
func TestFetchReviewsMapsWBResponse(t *testing.T) {
	...
	client := NewWithHTTPClient(config.WBConfig{Token: "token"}, "https://feedbacks-api.test", httpClient, 10, nil)
...
func TestPublishReplyPostsAnswer(t *testing.T) {
	...
	c := NewWithHTTPClient(config.WBConfig{Token: "tok"}, srv.URL, srv.Client(), 0, nil)
...
func TestFetchQuestionsMapsWBResponse(t *testing.T) {
	...
	c := NewWithHTTPClient(config.WBConfig{Token: "tok"}, srv.URL, srv.Client(), 10, nil)
...
func TestFetchReviewsRetriesWBThrottle(t *testing.T) {
	var call int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		call++
		if call == 1 {
...
func TestPublishReplyDoesNotRetryWBThrottle(t *testing.T) {
	var call int
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		call++
		w.Header().Set("X-Ratelimit-Retry", "5")
		w.WriteHeader(http.StatusTooManyRequests)
	}))
	defer srv.Close()

	c := NewWithHTTPClient(config.WBConfig{Token: "tok"}, srv.URL, srv.Client(), 0, nil)
	err := c.PublishReply(context.Background(), "fb-1", "x")
	if err == nil {
	...
	msg := err.Error()
	if !strings.Contains(msg, "Wildberries") || !strings.Contains(msg, "429") || !strings.Contains(msg, "5s") {
		t.Fatalf("error message = %q, want Wildberries/429/5s", msg)
	}
}
