...
func (s *Store) applyReviewFilters(ctx context.Context, query *gorm.DB, filter ReviewListFilter) (*gorm.DB, error) {
	query = query.Where("tenant_id = ?", TenantIDFromCtx(ctx))
