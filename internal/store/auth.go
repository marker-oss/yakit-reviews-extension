...
func (s *Store) CreateAdminUser(ctx context.Context, login, passwordHash string) (AdminUser, error) {
	user := AdminUser{TenantID: TenantIDFromCtx(ctx), Login: login, PasswordHash: passwordHash}
...
func (s *Store) CreateSession(ctx context.Context, token string, userID uint, tenantID uint, expiresAt time.Time) error {
	return s.db.WithContext(ctx).Create(&Session{Token: token, UserID: userID, TenantID: tenantID, ExpiresAt: expiresAt}).Error
}
