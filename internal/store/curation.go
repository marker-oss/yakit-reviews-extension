...
func (s *Store) ListReviewsWithCount(ctx context.Context, filter ReviewListFilter) ([]Review, int64, error) {
	...
	query, err = s.applyReviewFilters(ctx, query, filter)
	if err != nil {
		return nil, 0, err
	}
...
func (s *Store) DashboardStats(ctx context.Context) (Stats, error) {
	...
	active := db.Where("status <> ?", "deleted")
	if err := active.Count(&stats.TotalReviews).Error; err != nil {
		return Stats{}, err
	}

	visible := db.Session(&gorm.Session{}).
		Where("visibility = ?", "visible").
		Where("status <> ?", "deleted")
	if err := visible.Count(&stats.VisibleReviews).Error; err != nil {
		return Stats{}, err
	}
	if err := db.Session(&gorm.Session{}).Where("status = ?", "pending").Count(&stats.PendingReviews).Error; err != nil {
		return Stats{}, err
	}
	if err := db.Session(&gorm.Session{}).Where("status = ?", "deleted").Count(&stats.DeletedReviews).Error; err != nil {
		return Stats{}, err
	}
...
func (s *Store) SaveShowcaseRule(ctx context.Context, rule ShowcaseRule) error {
	rule.TenantID = TenantIDFromCtx(ctx)
	if rule.SortBy == "" {
		rule.SortBy = "recent"
	}
	if rule.Limit <= 0 {
...
func (s *Store) ListShowcasePins(ctx context.Context, article string) ([]ShowcasePin, error) {
	...
	err := s.db.WithContext(ctx).
		Where("tenant_id = ? AND seller_article = ?", TenantIDFromCtx(ctx), article).
		Order("position asc").
		Order("id asc").
		Find(&pins).Error
	return pins, err
...
func (s *Store) ReplaceShowcasePins(ctx context.Context, article string, reviewIDs []uint) error {
	return s.dirtyOnSuccess(ctx, s.db.WithContext(ctx).Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("tenant_id = ? AND seller_article = ?", TenantIDFromCtx(ctx), article).
...
func (s *Store) RemoveShowcasePin(ctx context.Context, article string, reviewID uint) error {
	return s.dirtyOnSuccess(ctx, s.db.WithContext(ctx).
		Where("tenant_id = ? AND seller_article = ? AND review_id = ?", TenantIDFromCtx(ctx), article, reviewID).
		...
		for externalID, article := range articles {
			...
			res := tx.Model(&Review{}).
				Where("tenant_id = ? AND marketplace = ? AND external_product_id = ? AND seller_article <> ?",
					TenantIDFromCtx(ctx), marketplaceID, externalID, article).
